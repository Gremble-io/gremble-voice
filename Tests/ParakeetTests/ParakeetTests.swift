...
final class ParakeetEngineTests: XCTestCase {
    ...
    func testTranscribeWithSourceThrowsWhenModelNotLoaded() async {
        let engine = ParakeetEngine()
        do {
            _ = try await engine.transcribe(samples: [0.0], source: .system)
            XCTFail("Expected ASREngineError.modelNotLoaded")
        } catch is ASREngineError {
            // expected
        } catch {
...
final class ParakeetStreamingEngineTests: XCTestCase {
    ...
    func testStreamingTranscribeWithSourceThrowsWhenModelNotLoaded() async {
        let engine = ParakeetStreamingEngine()
        do {
            _ = try await engine.transcribe(samples: [0.0], source: .system)
            XCTFail("Expected ASREngineError.modelNotLoaded")
    ...
    func testStartStreamingWithSourceThrowsWhenModelNotLoaded() async {
        let engine = ParakeetStreamingEngine()
        do {
            try await engine.startStreaming(config: .dictation, source: .system)
            XCTFail("Expected ASREngineError.modelNotLoaded")
        } catch is ASREngineError {
            // expected
        } catch {
